...
const PRIORITIES = [
    ...
    { value: "medium", label: "Med", color: "text-yellow-500", bg: "bg-yellow-500" },
...
export default function TasksPage() {
    ...
    if (loading) {
        return (<div className="flex items-center justify-center h-64"><div className="w-10 h-10 border-4 border-red-500 border-t-transparent rounded-full animate-spin" /></div>);
    }
        ...
        <div className="space-y-4 pb-24 md:pb-6 overflow-x-hidden">
            ...
            <div className="flex items-center justify-between gap-2">
                <h1 className="text-lg md:text-2xl font-bold flex items-center gap-2">
                    <div className="p-1.5 md:p-2 rounded-lg bg-gradient-to-br from-red-500 to-pink-600 text-white"><ListTodo className="w-4 h-4 md:w-5 md:h-5" /></div>
                    Tasks
                </h1>
                <button onClick={() => setShowForm(true)} className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-gradient-to-r from-red-500 to-pink-600 text-white font-semibold text-sm">
            ...
            <div className="grid grid-cols-3 gap-2">
                <div className="p-3 rounded-xl bg-card border border-border text-center"><p className="text-lg font-bold">{stats.total}</p><p className="text-[10px] text-muted-foreground">Total</p></div>
                <div className="p-3 rounded-xl bg-card border border-border text-center"><p className="text-lg font-bold text-yellow-500">{stats.pending}</p><p className="text-[10px] text-muted-foreground">Pending</p></div>
                <div className="p-3 rounded-xl bg-card border border-border text-center"><p className="text-lg font-bold text-green-500">{stats.completed}</p><p className="text-[10px] text-muted-foreground">Done</p></div>
            </div>

            {/* Filter - Grid layout to prevent overflow */}
            <div className="grid grid-cols-4 gap-1.5">
                {(["all", "todo", "in_progress", "done"] as FilterStatus[]).map((f) => (
                    <button key={f} onClick={() => setFilter(f)} className={`px-2 py-2 rounded-lg text-xs font-semibold transition-colors ${filter === f ? "bg-gradient-to-r from-red-500 to-pink-600 text-white" : "bg-secondary"}`}>
